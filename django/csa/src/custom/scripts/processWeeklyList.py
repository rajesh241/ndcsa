...
def addMemberTransaction(logger,myMember,productCode,productQuantity,productPrice,transactionType,orderDate,remarks=None):
  error = None
  ...
  if myProduct is None:
    ...
    myMemberTransaction=MemberTransaction.objects.filter(member=myMember,product=myProduct,orderDate=orderDate).first()
    myMemberTransaction.quantity=productQuantity
